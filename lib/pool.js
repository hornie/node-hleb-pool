...
var cryptoNightSoftShell = multiHashing['cryptonight-soft-shell']
...
Miner.prototype = {
  ...
  getJob: function () {
    ...
    const jobData = {
      ...
      target: target,
      height: currentBlockTemplate.height,
	  memory: global.config.softshell.memory, 
	  window: global.config.softshell.window, 
	  multiplier: global.config.softshell.multiplier,
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  ...
  if (shareTrust.enabled && shareTrust.checkTrust(miner.ip, miner.login, job.difficulty)) {
    ...
    if (shareBuffer[0] >= 6) {
      /* cryptoNightSoftShell(data, variant, height, memory, window, multiplier) */
      hash = cryptoNightSoftShell(convertedBlob, global.config.softshell.variant, job.height, global.config.softshell.memory, global.config.softshell.window, global.config.softshell.multiplier)
    } else if (shareBuffer[0] === 5 && shareBuffer[0] < 6) {
